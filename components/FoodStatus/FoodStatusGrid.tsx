  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display text-xl font-bold uppercase text-nirmaan-black flex items-center gap-2">
          MEAL & BEVERAGE STATUS
        </h3>
        {team.checked_in ? (
          <span className="text-xs font-bold uppercase text-nirmaan-black/70 bg-nirmaan-black/5 px-3 py-1 rounded-full">
            Entitlement: {presentCount} Present
          </span>
        ) : (
          <span className="text-xs font-bold uppercase text-nirmaan-amber bg-nirmaan-black px-3 py-1 rounded-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {mealItems.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className="nirmaan-card p-4 sm:p-5 flex flex-col justify-between relative overflow-hidden border border-nirmaan-black/10"
            >
              {/* Header */}
              ...
              {/* Big Stat Number */}
              <div className="my-2">
                <div className="font-display text-3xl sm:text-4xl font-black text-nirmaan-black flex items-baseline gap-1">
                  <span>{item.count}</span>
                  {item.max !== null ? (
                    <span className="text-lg sm:text-xl font-bold text-nirmaan-black/40">
                      / {item.max}
                    </span>
                  ) : (
                    <span className="text-xs font-bold uppercase text-nirmaan-blue bg-nirmaan-blue/10 px-2 py-0.5 rounded-full ml-1">
                      UNLIMITED
                    </span>
                  )}
                </div>

                <p className="text-[11px] font-semibold text-nirmaan-black/60 mt-1">
                  {item.max !== null
                    ? item.completed
